...
///
/// This complements ReadinessEngine's existing rolling-mean ACWR and Foster monotony. It does not
/// replace either one and it does not feed the Readiness level. The model keeps chronic load (CTL),
/// acute load (ATL), and their difference (TSB/form) in the SAME units as the supplied daily load.
/// NOOP currently supplies daily Effort/strain, so these values must not be described as TRIMP unless
/// the caller actually supplies TRIMP.
///
/// Model:
...
/// to seed both states, avoiding the artificial low bias produced by zero-seeding a new wearer.
///
/// Missing calendar days are NEVER silently filled with zero. A real rest day must be represented by a
/// row whose load is 0. A missing row or nil load breaks the contiguous suffix used for the model.
public enum TrainingLoadEngine {
    ...
    /// Calendar-day input. `load == nil` means the day has no usable load observation; that is different
    /// from `load == 0`, which is a measured rest day and remains part of the model.
    ...
    /// One chart-ready modeled day. The first point is the final priming day because CTL/ATL do not have
    /// a defined pre-seed state before then.
    ...
    public static func evaluate(days: [DailyLoad],
        ...
        // remains valid. Future rows and older history before the break do not affect this target's model.
        ...
        let suffix = suffixReversed.reversed()
        let contiguousDays = suffix.count
        guard contiguousDays >= configuration.minimumDays else {
            return unavailable(.notEnoughContiguousDays,
                               ...
                               startDay: suffix.first?.day,
                               endDay: suffix.last?.day)
        }

        let ordered = Array(suffix)
        ...
        if ordered.count > configuration.primeDays {
    ...
    /// Convenience for already-dense load arrays used by analytics tests and non-calendar callers.
    /// Day labels are synthetic but deterministic; the model math is identical to `evaluate(days:)`.
    ...
    /// timezone-free so Swift/Kotlin twins cannot drift around DST or locale boundaries.
