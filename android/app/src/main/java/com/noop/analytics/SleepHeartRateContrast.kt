...
/*
 * SleepHeartRateContrast.kt - descriptive primary-sleep vs wake HR contrast.
 * Behavioral twin of StrandAnalytics/SleepHeartRateContrast.swift.
 *
 * This is NOT another resting-heart-rate definition. It compares explicitly
 * supplied fixed-grid wake and primary-sleep HR windows and leaves NOOP's
 * shipped floor and primary-session-mean RHR work untouched.
 *
 * Descriptive wellness context only. No dipper/non-dipper/riser, risk, or
 * diagnostic label is produced.
 */
object SleepHeartRateContrast {
     ...
     *
     * Each list represents expected equal-duration epochs. null is an
     * unobserved epoch, so valid/total is an actual coverage fraction rather
     * than a sample-cadence guess. Values outside 30..220 bpm are ignored.
     *
     * Means are unweighted across valid epochs. Callers with irregular raw
     * samples should first aggregate to a declared fixed cadence.
     */
    fun evaluate(
        wakeHR: List<Double?>,
